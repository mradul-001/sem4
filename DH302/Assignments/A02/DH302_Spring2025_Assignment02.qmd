...
df <- rbind(df.breast, df.control) # combining the dataframes for plotting
```
...
df_numeric <- df[, 4:21] 
df$food_category <- substr(df$food_code, 1, 1)
...
scale_color_gradient2(low = "pink", mid = "violet", high = "blue", 
                      midpoint = mean(var_contrib$contrib)) +
  theme
```

-   PC1 might be differentiating the food on the basis of their pH levels, since acidic amino acids are at one side of the axis and non-acidic are at the other side. Or the PC1 may be classifying the food basis of the total protein content. Low protein food is on the left side of the axis, while high protein food on the right side.

-   PC2 may capture differences related to polarity and specific side chain properties. Hydrophobic amino acids are at the top, while more polar or structurally unique amino acids are at the bottom. PC2 may also be capturing differences in structural flexibility, highly flexible amino acids like glycine are at the lower side, while rigid amino acids like valine and leucine are at the upper side.
...
topPC1 <- var_contrib[order(-var_contrib[,1]), ][1:2, ]
topPC2 <- var_contrib[order(-var_contrib[,2]), ][1:2, ]

print("Top 2 features for PC1:")
print(topPC1)
cat("\n\n")
print("Top 2 features for PC2:")
print(topPC2)

```

-   The amino acids contributing maximum towards PC1 are: **Methionine** and **Lysine**.

-   The amino acids contributing maximum towards PC2 are: **Cystine** and **Alanine**.
